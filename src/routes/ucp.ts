...
import type { BigCommerceAdapter } from "../adapter";
import type {
  Cart,
  Order,
  Product,
...
export function buildUcpRouteTable(adapter: BigCommerceAdapter): RouteTable<RouteHandler> {
...
function catalogSearch(adapter: BigCommerceAdapter): RouteHandler {
...
function catalogLookup(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function createCart(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getCart(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
...
function updateCart(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return bad(400, "missing_id", "cart id required in path");
...
function checkout(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getOrder(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
