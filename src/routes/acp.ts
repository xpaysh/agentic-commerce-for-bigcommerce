...
const sessions = createSessionStore({
  pluginId: process.env.ACP_SESSION_PLUGIN_ID || "agentic-commerce-for-bigcommerce",
});
...
export function buildAcpRouteTable(adapter: BigCommerceAdapter): RouteTable<RouteHandler> {
...
function createSession(adapter: BigCommerceAdapter): RouteHandler {
...
function updateSession(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getSession(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function completeSession(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function getOrderRoute(adapter: BigCommerceAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
